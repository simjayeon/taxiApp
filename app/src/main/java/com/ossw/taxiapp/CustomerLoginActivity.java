                ...
                mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(CustomerLoginActivity.this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if(!task.isSuccessful()){
                            Toast.makeText(CustomerLoginActivity.this, "회원가입 오류", Toast.LENGTH_SHORT).show();
                        }else{
                            String user_id = mAuth.getCurrentUser().getUid();
                            DatabaseReference current_user_db = FirebaseDatabase.getInstance().getReference().child("Users").child("Customers").child(user_id);
                            current_user_db.setValue(true);
                            Toast.makeText(CustomerLoginActivity.this, "회원가입 성공", Toast.LENGTH_SHORT).show();
                            Toast.makeText(CustomerLoginActivity.this, "로그인되었습니다.", Toast.LENGTH_SHORT).show();
                            /*
                            Intent intent = new Intent(CustomerLoginActivity.this, CustomerMenuActivity.class);
                            startActivity(intent);
                            finish();
        ...
        c_login.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(CustomerLoginActivity.this, new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if(!task.isSuccessful()){
                            Toast.makeText(CustomerLoginActivity.this, "잘못된 아이디 및 패스워드입니다.", Toast.LENGTH_SHORT).show();
                        }else{
